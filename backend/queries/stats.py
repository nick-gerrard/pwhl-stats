...
async def get_player_info(conn: AsyncConnection, season_id: int, player_id: int) -> dict:
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute(
...
async def get_goalie_info(conn: AsyncConnection, season_id: int, player_id: int) -> dict:
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute(
            """SELECT p.first_name, p.last_name, t.name as team_name,
